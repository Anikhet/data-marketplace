...
const faqItems = [
  {
    ...
    answer:
      "This list includes verified professional contacts with Name, Title, Company, and Email. Phone numbers are available as an enrichment.",
  },
  {
    question: "How fresh is the data?",
    answer:
      "The data freshness is indicated in the listing details (e.g., Updated Weekly, Updated Daily). We strive to provide the most up-to-date information possible.",
  },
  {
    question: "How is the data verified?",
    answer:
      "Our data is verified through a multi-step process including automated checks and manual review by our team. The verification rate is displayed in the stats section.",
  },
  {
    question: "What is the exclusivity level?",
    answer:
      "Exclusivity levels indicate how widely the list is available. Premium lists are offered to a limited number of buyers to maintain their value.",
  },
];
...
export default function ListingPreview({ listing }: ListingPreviewProps) {
  ...
  const suggestedLists: Listing[] = [
    ...
    },
    {
      ...
      description:
        "Find lists where phone numbers are available as an enrichment.",
      price: 0,
      isVerified: true,
  ...
  const baseColumns: ColumnDef<PreviewRecord>[] = [
    ...
    },
    {
      ...
      accessorKey: "email",
      cell: (info) =>
        info.getValue<string>().replace(/(?<=.{3}).(?=.*@)/g, "*"),
    },
  ];
  ...
  const columns = selectedColumns.includes("phone")
    ? [
        ...baseColumns,
        {
          header: "Phone",
          accessorKey: "phone",
          cell: (info: CellContext<PreviewRecord, string | null>) =>
            info.getValue() ?? "N/A",
        },
      ]
    : baseColumns;
  ...
  const handleRequestList = async () => {
    ...
    try {
      ...
      const message =
        err instanceof Error ? err.message : "Failed to request list";
      setError(message);
      toast.error(message);
    } finally {
      setIsLoading(false);
...
<motion.div
  ...
  initial="hidden"
  animate="visible"
  className="p-8 border-b border-gray-100 bg-white/70 backdrop-blur-md shadow-sm"
>
  <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-6">
    ...
    <div className="flex-1 min-w-0">
      ...
      >
        ...
        <motion.div variants={badgeVariants}>
          <Badge
            variant="secondary"
            className="flex items-center px-3 py-1.5 rounded-full bg-emerald-100 text-emerald-800 border border-emerald-200 shadow-sm"
          >
    ...
    >
      <Badge
        variant="outline"
        className="flex items-center px-5 py-2 rounded-full bg-amber-100/60 text-amber-800 border border-amber-300/50 shadow-sm backdrop-blur-md"
      >
              ...
              <div className="flex items-center space-x-4">
                {stat.progress ? (
                  <>
                    <Progress
                      value={stat.value}
                      className="w-32 h-1.5 bg-gray-100/50"
                    />
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-10 mb-10">
          <motion.div variants={itemVariants}>
            <h2 className="text-3xl font-bold text-gray-900 mb-4">
              Instant Access to Premium Contacts
            </h2>
            <p className="text-gray-600 text-lg">
              Exclusive verified contacts with instant enrichment options
            </p>
          </motion.div>
          ...
          <motion.div variants={itemVariants} className="mb-8">
            <div className="flex items-center justify-between text-sm text-gray-600 mb-4">
              ...
              <span className="font-medium">
                {listing.stats.remainingCount} spots left
              </span>
            </div>
            <Progress
              value={
                ((listing.stats.totalCount - listing.stats.remainingCount) /
                  listing.stats.totalCount) *
                100
              }
